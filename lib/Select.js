...
var Select = _createReactClass2['default']({
	...
	handleKeyDown: function handleKeyDown(event) {
		...
		switch (event.keyCode) {
			...
			// break;
			// case 186: // ;
			// 	if (this.props.allowCreate && this.props.multi && this.props.newOptionCreator && this.state.inputValue) {
			// 		 event.preventDefault();
			// 		 event.stopPropagation();
	...
	createNewOption: function createNewOption() {
		return this.selectValue(this.props.newOptionCreator(this.state.inputValue));
	},

	selectFocusedOption: function selectFocusedOption() {
		if (!this.props.isLoading && this.props.allowCreate && !this._focusedOption &&
			this.props.newOptionCreator && this.state.inputValue) {
			var existingChoice = this._visibleOptions.filter(function(opt) {
				var input = this.state.inputValue && this.state.inputValue.trim().toLowerCase();
				var label = opt.l && opt.l.trim().toLowerCase();
				return input === label;
			}.bind(this))[0];
			if (existingChoice) {
				this._focusedOption = existingChoice;
	...
	renderMenu: function renderMenu(options, valueArray, focusedOption) {
		...
		if (!focusedOption) {
			...
			if (existingChoice) {
				focusedOption = this._focusedOption = existingChoice;
			}
		}
